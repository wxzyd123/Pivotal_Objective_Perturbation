...
def main():
    ...
    for epoch in range(1, hps.train.epochs + 1):
        loss_disc_all, loss_gen_all = protected_train(hps, [net_g, net_d], [optim_g, optim_d], 
                                                      train_loader, scaler, noises, is_fixed)
    ...
    save_path = f"./checkpoints/{model_name}_{mode}_{dataset_name}_{epoch}.pth"
    torch.save(net_g.state_dict(), save_path)
    print(f"Saving the checkpoint to {save_path}.")
    evaluation(net_g, config_path, model_name, dataset_name, mode, device)


def protected_train(hps, nets, optims, train_loader, scaler, noises, is_fixed):
    net_g, net_d = nets
    ...
    for batch_index, batch in enumerate(train_loader):
        ...
        noise = noises[batch_index].to(device)

        p_wav = torch.clamp(wav + noise, min=-1., max=1.)
        p_spec, spec_len = get_spec(hps.data, p_wav, wav_len)

        wav_hat, l_length, attn, ids_slice, x_mask, z_mask, \
                (z, z_p, m_p, logs_p, m_q, logs_q) = net_g.forward(text, text_len, p_spec, spec_len, speakers, is_fixed=is_fixed)
